...
from .hive_request import Blockchain
from .config import domain
steem = Blockchain()

def search_community(filter, offset):            
        inline_results = []
        try:
            results = steem.get_hive_community(filter) 
...
def view_communit_post(community, offset):            
        inline_results = []
        try:
            results = steem.get_hive_community_post(community)
            MAX_RESULTS_PER_PAGE = 20
            paginated_results = results[offset:offset + MAX_RESULTS_PER_PAGE]
