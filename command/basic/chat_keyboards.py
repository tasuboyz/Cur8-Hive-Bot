...
class Keyboard_Manager:
    def __init__(self):
        self.db = Database()
        self.postpage_url = 'https://tasuboyz.github.io/Hive-TWA-Posting/'
    ...
    def create_start_reply_keyboard(self, message=None):
        ...
        if result:
            username = result[0]
            wif = result[1]
            keyboard.append([KeyboardButton(text=send_post_text, web_app=WebAppInfo(url=f"{self.postpage_url}?username={username}&wif={wif}"))])
    ...
    def setting_list(self, data):
        ...
        voting_power = self.language.vote_weight(language_code)
        result = self.db.get_user_account(user_id)  
        if result:  
            username = result[0] 
            if username:                       
                ...
                keyboard.append([KeyboardButton(text=change_account_text, web_app=WebAppInfo(url=self.SignIn_url)), KeyboardButton(text=voting_power)])
        else:
    ...
    def view_community_post(self, community, language_code, username_sub=[]):
        ...
        keyboard = []
        if username_sub is not None:
            if community in username_sub:
                keyboard.append([InlineKeyboardButton(text="unsubscribe 🚫", callback_data="community:unsub")])
            else:
                keyboard.append([InlineKeyboardButton(text="subscribe ✅", callback_data="community:sub")])
    ...
    def give_like(self, count=0):
        keyboard = []
        keyboard.append([InlineKeyboardButton(text=f"{count}❤️", callback_data=f"❤️: {count}")])
        keyboard = InlineKeyboardMarkup(inline_keyboard=keyboard)
        return keyboard
    
    def set_voting_power(self):
        keyboard = []
        for i in range(10, 101, 10):
            keyboard.append([InlineKeyboardButton(text=f"{i}%", callback_data=f"voting: {i}")])
        keyboard = InlineKeyboardMarkup(inline_keyboard=keyboard)
        return keyboard