...
class Database():
    ...
    def create_table(self):
        ...
        self.c.execute(f'''CREATE TABLE IF NOT EXISTS {self.USER_POWER_VOTE} ({self.user_id} INTEGER PRIMARY KEY, {self.power_vote} INT)''')
        self.c.execute(f'''
    ...
    def get_user_account(self, user_id):
        ...
        if result:
            ...
            if any(char.isupper() for char in account):
                ...
                self.c.execute(f'''UPDATE {self.USER_ACCOUNT} SET {self.account} = ? WHERE {self.user_id} = ?''', (account, user_id))
                self.conn.commit()
            return result
        else:
            return None
    ...
    def delete_program_post_data(self, datetime_str):
        # Converti la stringa datetime in un oggetto datetime
        datetime_obj = datetime.strptime(datetime_str, '%Y-%m-%dT%H:%M')
        
        self.c.execute(f'''
        DELETE FROM {self.PROGRAM_POST} 
        WHERE strftime('%Y-%m-%dT%H:%M', {self.datetime}) < ?
        ''', (datetime_str,))
    ...
    def insert_user_power_vote(self, user_id, power_vote):
        self.c.execute(f'''INSERT OR REPLACE INTO {self.USER_POWER_VOTE} ({self.user_id}, {self.power_vote}) VALUES (?, ?)''', (user_id, power_vote))
        self.conn.commit()

    def delete_user_power_vote(self, user_id):
        self.c.execute(f'''DELETE FROM {self.USER_POWER_VOTE} WHERE {self.user_id} = ?''', (user_id,))
        self.conn.commit()

    def get_user_power_vote(self, user_id):
        self.c.execute(f'''SELECT {self.power_vote} FROM {self.USER_POWER_VOTE} WHERE {self.user_id} = ?''', (user_id,))
        result = self.c.fetchone()
        self.conn.commit()
        return result[0] if result else None
