...
class Admin_Commands:
    ...
    async def clean_inactive_users(self, callback_query: CallbackQuery):       
        ...
        id_to_delate = []
        try:
            counter = await callback_query.message.answer(f"{count}") # type: ignore
    ...
    async def wait_document(self, callback_query: CallbackQuery, state: FSMContext):
        await callback_query.message.answer("Send file txt:") # type: ignore
    ...
    async def send_me_username(self, callbackquery: CallbackQuery, state: FSMContext):
        await state.set_state(Form.set_steem_username)
        await callbackquery.message.reply("send username:") # type: ignore

    async def save_steem_account(self, message: Message, state: FSMContext):
        await state.clear()
        steem_username = message.text
        try:
            result = self.beem.get_profile_info(steem_username)
            self.db.insert_steem_username(steem_username)
            await message.answer(f"{steem_username} saved!")
        except Exception as e:     
            await bot.send_message(self.admin_id, str(e))
